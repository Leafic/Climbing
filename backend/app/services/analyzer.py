...
from app.services.climbing_knowledge import get_knowledge_prompt
...
completionProbability는 '다음 시도 예상 성공 확률'입니다."""
        ...
        # 도메인 지식 주입
        knowledge_block = get_knowledge_prompt(skill_level, attempt_result)
...
[★★ 전문가 분석 프레임워크 ★★]
아래는 프로 클라이밍 코치가 영상을 분석할 때 사용하는 체크리스트와 기술 레퍼런스입니다.
이 프레임워크를 기반으로 영상을 분석하세요. 해당되는 항목이 있으면 반드시 결과에 반영하세요.

...
[클라이밍 짐 홀드 식별 핵심 지식]
실내 클라이밍 짐에서 루트(문제)는 **같은 색상의 홀드** 또는 **같은 색상의 테이프/태그**로 표시됩니다.
- **홀드 자체가 같은 색상**인 경우: 벽에 여러 색상의 홀드가 섞여 있고, 같은 색 홀드만 사용하는 것이 하나의 루트입니다.
- **테이프/태그로 표시**된 경우: 홀드 자체 색은 다양하지만, 홀드 옆이나 위에 같은 색 테이프(접착 테이프, 스티커, 태그)가 붙어 있어 루트를 구분합니다.

[시작 홀드와 탑 홀드 판별 — 매우 중요]
- **시작 홀드(스타트)**: 벽의 가장 아래쪽에 위치한 해당 색상 홀드. 보통 **테이프 2개** 또는 **'S' 표시**, 또는 **화살표 표시**가 있습니다. 시작 홀드가 2개(양손)인 경우도 많습니다.
- **탑 홀드(완등 지점)**: 벽의 가장 위쪽에 위치한 해당 색상 홀드. 보통 **'TOP' 표시** 또는 **테이프 표시**가 있습니다.
- 시작과 탑을 모르겠으면, **해당 색상 홀드 중 가장 아래쪽 = 시작, 가장 위쪽 = 탑**으로 판단하세요.
- 발 전용 홀드(풋홀드)도 같은 색 테이프로 표시될 수 있습니다 — 이것도 루트에 포함시키세요.

[분석 규칙]
1. 사진에서 **{hold_color}** 색상의 홀드 또는 **{hold_color}** 색 테이프/태그가 붙은 홀드를 **모두** 식별하세요.
2. 식별한 홀드를 위치(높이)별로 정렬하여 아래→위 순서로 루트를 구성하세요.
3. 가능한 루트 {route_count}를 제안하세요.
4. 각 루트에 대해:
   - holds 배열에 각 홀드의 좌표를 작성 (xPct: 이미지 왼쪽에서 0~100%, yPct: 이미지 위쪽에서 0~100%)
   - 반드시 **가장 아래 홀드를 "시작"**, **가장 위 홀드를 "탑"**으로 지정하세요
   - 중간 홀드는 아래→위 순서로 "2", "3", "4" ... 번호를 매기세요
   - 시작 홀드부터 탑 홀드까지의 이동 순서를 steps에 작성
   - {step_detail}
   - approachStrategy에 루트 공략 전략을 작성
   - keyTips에 핵심 팁 2~3개 작성
5. 루트 난이도를 V-스케일로 추정하세요.
6. 사진에서 관찰할 수 없는 내용은 추측하지 마세요.
7. holds의 좌표는 사진에서 실제 홀드가 보이는 위치를 **정확히** 반영하세요. 대충 균등 분포로 찍지 마세요.

[주의사항]
- 같은 색이라도 **다른 루트에 속하는 홀드**가 있을 수 있습니다 (벽의 다른 섹션). 물리적으로 연결 가능한 홀드만 하나의 루트로 묶으세요.
- 홀드의 모양(크림프, 슬로퍼, 저그, 핀치, 포켓 등)도 식별 가능하면 steps에서 언급하세요.

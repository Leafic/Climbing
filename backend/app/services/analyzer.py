...
사용자가 도전하려는 홀드: **{hold_color}**
사용자 숙련도: {rcfg["label"]}

[★★★ 한국 클라이밍 짐 루트 식별법 — 반드시 숙지 ★★★]

한국 실내 클라이밍 짐에서 루트(문제)를 구분하는 방법은 여러 가지가 있습니다.
사진을 먼저 **전체적으로** 관찰하고, 이 짐이 어떤 방식을 사용하는지 판별하세요.

**방식 1: 숫자 스티커/태그 시스템 (가장 흔함)**
- 홀드 옆이나 벽에 **동그란 스티커(원형 태그)**가 붙어 있고, 안에 **숫자**가 적혀 있습니다.
- 예: 빨간 동그라미 안에 "6", 파란 동그라미 안에 "5" 등
- **같은 숫자의 스티커가 붙은 홀드들이 하나의 루트**입니다.
- 숫자는 보통 난이도를 나타냅니다 (1=가장 쉬움, 10=매우 어려움).
- 스티커 색상은 난이도 등급을 나타내기도 합니다 (빨강=어려움, 파랑=중급 등).
- 이 경우 사용자가 입력한 "{hold_color}"은 **숫자** 또는 **스티커 색상+숫자** 또는 **홀드 색상**일 수 있습니다.

**방식 2: 같은 색상 홀드 시스템**
- 벽에 여러 색상의 홀드가 섞여 있고, **같은 색 홀드끼리 하나의 루트**를 구성합니다.
- 예: 노란 홀드만 따라가면 하나의 루트, 파란 홀드만 따라가면 다른 루트

**방식 3: 테이프 시스템**
- 홀드 색은 다양하지만, 홀드 옆에 **같은 색 테이프(접착 테이프)**가 붙어 있습니다.
- 같은 색 테이프가 붙은 홀드들이 하나의 루트

[★ 시작(스타트) 홀드 판별 — 매우 중요 ★]
스타트 홀드를 정확히 찾는 것이 루트 분석의 핵심입니다.
- **숫자 스티커 시스템**: 해당 숫자 스티커가 붙은 홀드 중 **가장 아래쪽**에 있는 1~2개가 스타트. 스티커가 2개 겹쳐 붙어있거나, 'S' 표시가 있을 수 있습니다.
- **색상 시스템**: 해당 색 홀드 중 **가장 아래쪽** 1~2개가 스타트. 테이프가 2개 붙어있을 수 있습니다.
- 볼더링에서 스타트는 보통 **양손으로 잡고 시작** → 스타트 홀드가 2개(양손용)인 경우가 많습니다.

[★ 탑(완등) 홀드 판별 ★]
- 해당 루트의 홀드 중 **가장 위쪽**에 있는 홀드가 탑
- 'TOP', 'G(골)', 또는 특별한 표시가 있을 수 있음
- 이 홀드를 양손으로 잡으면 완등

[분석 순서 — 이 순서대로 진행하세요]
1. **사진 전체를 관찰**하여 이 짐이 어떤 루트 표시 방식을 쓰는지 판별하세요.
2. 사용자가 입력한 "{hold_color}"에 해당하는 홀드를 **모두** 찾으세요.
   - 숫자 스티커 시스템이면: 해당 숫자/색상 스티커가 붙은 홀드를 모두 찾기
   - 색상 시스템이면: 해당 색상 홀드를 모두 찾기
3. 찾은 홀드를 **아래→위 높이순**으로 정렬하세요.
4. 가장 아래 = 스타트, 가장 위 = 탑으로 설정하세요.
5. 루트 {route_count}를 제안하세요.

[각 루트 작성 규칙]
- holds 배열: 각 홀드의 좌표 (xPct: 왼쪽에서 0~100%, yPct: 위에서 0~100%)
- 반드시 **가장 아래 홀드 = "시작"**, **가장 위 홀드 = "탑"** 지정
- 중간 홀드: 아래→위 순서로 "2", "3", "4" ...
- steps: 스타트부터 탑까지 이동 순서. {step_detail}
- approachStrategy: 루트 공략 전략
- keyTips: 핵심 팁 2~3개
- difficulty: V-스케일 추정

[좌표 정확도 — 매우 중요]
- holds의 좌표는 사진에서 **실제 홀드가 보이는 위치를 정확히** 반영하세요.
- 홀드를 3~4개만 대충 잡지 말고, 해당 루트에 속하는 **모든 홀드를 빠짐없이** 포함하세요.
- 대충 균등 분포로 찍지 마세요. 실제 위치를 보고 작성하세요.

[주의사항]
- 같은 색/숫자라도 벽의 완전 다른 섹션에 있으면 별도 루트일 수 있습니다.
- 홀드 모양(크림프, 슬로퍼, 저그, 핀치, 포켓, 볼륨 등) 식별 가능하면 steps에서 언급하세요.
- 사진에서 관찰할 수 없는 내용은 추측하지 마세요.
